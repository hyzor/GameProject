...
void PlayerMulti::Update(float dt, float gameTime, DirectInput* dInput, World* world, std::vector<Player*>* multiplayers)
{
	...
	// Update which keyframes to play
	mEntity->SetKeyFrameInterval(mAnimations[mCurAnim].FrameStart, mAnimations[mCurAnim].FrameEnd);

	// Only running backwards plays an animation backwards
	if (mCurAnim == RunningBackwardsAnim)
		mEntity->PlayAnimationBackwards();
	else
		mEntity->PlayAnimationForwards();
}
...
void PlayerMulti::HandelPackage(Package *p)
{
	...
	else if(p->GetHeader().operation == 11)
	{
		...
		if (mWeapons.at(mCurWeaponIndex)->FireProjectile(p, XMFLOAT3(-d.x, -d.y, -d.z), hitPoint))
		{
